...
$GTID = ""; // place holder for product id information
...
if (isset($_GET['GTID'])) { // note that the spelling 'pid' is based on the query string variable name

	// product id available, validate the information, then compose a query accordingly to retrieve information.

	$GTID = intval($_GET['GTID']); // force all input into an integer.  If the input is a string or empty, it will be converted to 0.

	// validate the product id -- check to see if it is greater than 0
		if ($GTID>0 ){
			//compose the query
			$sql = "DELETE from GivingThanks WHERE GTID = ?"; // note that the spelling "PID" is based on the field name in the database product table.
...
<div class='container'>
    ...
    <div class='col-10 mx-auto'>
        ...
        <p class='text-center'>Back to the <a href='giving-thanksShow.php'>Giving Thanks Page</a></p>
    </div>

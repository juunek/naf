...
$ResID = ""; // place holder for product id information
...
if (isset($_GET['ResID'])) { // note that the spelling 'pid' is based on the query string variable name

	// product id available, validate the information, then compose a query accordingly to retrieve information.

	$ResID = intval($_GET['ResID']); // force all input into an integer.  If the input is a string or empty, it will be converted to 0.

	// validate the product id -- check to see if it is greater than 0
		if ($ResID>0 ){
			//compose the query
			$sql = "DELETE from Resources WHERE ResID = ?"; // note that the spelling "PID" is based on the field name in the database product table.

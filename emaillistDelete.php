...
$ELID = ""; // place holder for product id information
...
if (isset($_GET['ELID'])) { // note that the spelling 'pid' is based on the query string variable name

	// product id available, validate the information, then compose a query accordingly to retrieve information.

	$ELID = intval($_GET['ELID']); // force all input into an integer.  If the input is a string or empty, it will be converted to 0.

	// validate the product id -- check to see if it is greater than 0
		if ($ELID>0 ){
			//compose the query
			$sql = "DELETE from NAFEmailList WHERE ELID = ?"; // note that the spelling "PID" is based on the field name in the database product table.

			$stmt = $conn->stmt_init();

			if ($stmt->prepare($sql)){

				$stmt->bind_param('i',$ELID);
	...
	$output = "<h2 class='text-center text-danger my-5'>Error</h2><p class='text-center'>To manage email list records, please follow the link below to visit the admin page.  Thank you. </p>";
}
...
<div class='container'>
    ...
    <div class='col-10 mx-auto'>
        ...
        <p class='text-center'>Back to the <a href='emaillistShow.php'>Manage Email List Subscriptions Page</a></p>
    </div>

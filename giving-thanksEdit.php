...
if (isset($_POST['Submit'])) {
	...
	// define constant for upload folder
	  ...
	  $detectedType1 =exif_imagetype($_FILES['GTImage']['tmp_name']);
		$detectedType2 =is_uploaded_file($_FILES['GTAdditionalImage']['tmp_name']);

	  if (in_array($detectedType1, $allowedTypes))  {

	    trim($_FILES['GTImage']['name']);
	    $_FILES['GTImage']['name']= str_replace(" ", "_", $_FILES['GTImage']['name']);
	    // move the file to the upload folder and rename it
	    if (move_uploaded_file($_FILES['GTImage']['tmp_name'], UPLOAD_DIR.$_FILES['GTImage']['name'])){

	        $GTImage = $_FILES['GTImage']['name'];
		...
		if (!empty($uploadMessage)) {
			if($_POST['GTExists'] == 'No'){
				$uploadMessage = "<ul><li>Image File $uploadMessage</li></ul>";
			}else {
				$GTImage = $_POST['GTExists'];
				$uploadMessage = "";
			}
		}
		...
		if (in_array($detectedType2, $allowedTypes))  {

	    trim($_FILES['GTAdditionalImage']['name']);
	    $_FILES['GTAdditionalImage']['name']= str_replace(" ", "_", $_FILES['GTAdditionalImage']['name']);
	    // move the file to the upload folder and rename it
	    if (move_uploaded_file($_FILES['GTAdditionalImage']['tmp_name'], UPLOAD_DIR.$_FILES['GTAdditionalImage']['name'])){

	        $GTAdditionalImage = $_FILES['GTAdditionalImage']['name'];
		...
		if (empty($detectedType2)) {
			$uploadMessage='';
			$GTAdditionalImage="";
		}
		if (!empty($uploadMessage)) {
			if($_POST['AdditionalExists'] == 'No'){
				$uploadMessage = "<ul><li>Additional Image File $uploadMessage</li></ul>";
			}else {
				$GTAdditionalImage = $_POST['AdditionalExists'];
				$uploadMessage = "";
			}
		}
	//validate user input
	...
	$required = array("GTHeader", "GTDetails"); // note that, in this array, the spelling of each item should match the form field names

	// set up the expected array
	$expected = array("GTID", "GTAdditionalImageAltText","GTHeader", "GTDetails", "GTPersonName", "GTPersonDetails", "GTExists", "AdditionalExists"); // again, the spelling of each item should match the form field names

    // set up a label array, use the field name as the key and label as the value
  $label = array ("GTID" => "Giving Thanks ID (GTID)", "GTAdditionalImageAltText"=>'Additional Image Details', "GTHeader"=>'Article Title', "GTDetails"=>'Article Information', "GTPersonName"=>'Donor Name', "GTPersonDetails"=>'Donor Information', "GTExists"=>'Image', "AdditionalExists"=>'Additional Image');
	...
	if (empty($missing) && empty($uploadMessage)){
		...
		if ($GTID != "") {
			...
			$sql = "Update GivingThanks SET GTImage = ?, GTAdditionalImage = ?, GTAdditionalImageAltText = ?, GTHeader = ?, GTDetails = ?, GTPersonName = ?, GTPersonDetails = ? WHERE GTID = ?";

			if($stmt->prepare($sql)){

				// Note: user input could be an array, the code to deal with array values should be added before the bind_param statment.
				$stmt->bind_param('sssssssi',$GTImage, $GTAdditionalImage, $GTAdditionalImageAltText, $GTHeader, $GTDetails, $GTPersonName, $GTPersonDetails, $GTID);
			...
			$sql = "Insert Into GivingThanks (GTImage, GTAdditionalImage, GTAdditionalImageAltText, GTHeader, GTDetails, GTPersonName, GTPersonDetails) values (?, ?, ?, ?, ?, ?, ?)";

			if($stmt->prepare($sql)){

				// Note: user input could be an array, the code to deal with array values should be added before the bind_param statment.

				$stmt->bind_param('sssssss',$GTImage, $GTAdditionalImage, $GTAdditionalImageAltText, $GTHeader, $GTDetails, $GTPersonName, $GTPersonDetails);
				$stmt_prepared = 1; // set up a variable to signal that the query statement is successfully prepared.
			}
		}
		...
		if ($stmt_prepared == 1){
			if ($stmt->execute()){
				...
				$output .= "<p><a href='giving-thanksShow.php'><button class='btn btn-naf-blue w-100 mt-5'>BACK TO THE GIVING THANKS MANAGEMENT PAGE</button></a></p>";
			} else {
				//$stmt->execute() failed.
		...
		if ($GTID != "") {
			$output .= "</ul><a href='giving-thanksForm.php?GTID=$GTID'> <p>$uploadMessage</p>\n <button class='btn btn-naf-blue w-100 mt-5'>BACK TO THE GIVING THANKS FORM PAGE</button></a></div>\n";
		}else {
			$output .= "</ul><a href='giving-thanksForm.php'><button class='btn btn-naf-blue w-100 mt-5'>BACK TO THE GIVING THANKS FORM PAGE</button></a></div>\n";
		}
	}
}

} else {
	$output = "<h2 class='text-center text-danger my-5'>Invalid Request</h2><p class='text-center'>Please begin your giving thanks managment operation from the <a href='giving-thanksShow.php'>Giving Thanks Page</a>.</p>";
}
